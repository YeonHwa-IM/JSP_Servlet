		...
		<% for(int i= 0; i < blist.size(); i++){
			Board b = blist.get(i); %>
				...
				<div class="thumb-list" align="center">
					<div>
						<input type="hidden" value="<%= b.getbId() %>"><!--나중을 생각해서 상세보기에 쓰기위해 히든해놓기  -->
						<% for(int j=0; j < flist.size(); j++){
							Attachment a= flist.get(j);
							
							if(b.getbId() == a.getbId()){ %><!-- b와 a에 bId가 같다는 뜻은 같은 게시글에 달려있는 사진이라는 뜻! 같은 아이디어야 아래에 이미지가 들어간다!-->
								<img src="<%= request.getContextPath() %>/thumbnail_uploadFiles/<%= a.getChangeName() %>" width="200px"	height="150px">
							<% } %>
						<% } %>
					</div>
					<p>	No. <%= b.getbId() %> <%= b.getbTitle() %><br> <!-- 아이디와 제목 넣기  -->
						조회수 : <%= b.getbCount() %> <!-- 조회수 넣기 -->
					</p>
				</div>
			<% } %>
		</div>
