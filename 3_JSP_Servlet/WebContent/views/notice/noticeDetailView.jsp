			...
			<form action="views/notice/noticeUpdateForm.jsp" id="detailForm" name="detailForm">
				<table>
					<tr>
						...
						<!--noticeUpdateForm으로 보낼때 공지사항 번호를 같이 보내야
						 다시 servlet으로 공지사항 번호를 보낼 수 있으므로
						 hidden에 같이 넣어둠  -->
...
<%-- 						<%= notice.getnWriter() %> --%>
				...
				<div align="center">
					...
					<button type="submit" id="updateNoBtn">수정</button><!--어드민한테만 보이게끔 만들어줘야함  -->
	...
	<script>
		function deleteNo(){
			...
			if(bool){
				location.href="<%= request.getContextPath() %>/delete.no?no=" + <%= notice.getnNO() %>; //nNo가 아닌건 내 오타니까 에러 아님~!
			}
		}
	</script>
